<script lang="ts">
	...
	import { Building2, User, Calendar, ArrowLeft, ExternalLink, MapPin, Package, Grid3x3 } from 'lucide-svelte';
...
<div class="space-y-6">
	...
	<div class="flex items-center gap-2 text-sm">
		...
		<a href="/admin/settlements" class="text-surface-600 dark:text-surface-400 hover:text-primary-500">Settlements</a>
		<span class="text-surface-400">/</span>
		...
		<div class="card preset-filled-surface-100-900 p-6">
			...
			<div class="space-y-4">
				{#if settlement.playerProfile}
					<div>
						<div class="text-sm text-surface-600 dark:text-surface-400 font-medium mb-1">Player</div>
			...
			{#if settlement.storage && Object.keys(settlement.storage).length > 0}
				<div class="space-y-2">
					{#each Object.entries(settlement.storage) as [resource, amount]}
						<div class="flex items-center justify-between p-2 bg-surface-200 dark:bg-surface-700 rounded">
	...
	{#if settlement.plot && settlement.plot.length > 0}
		<div class="card preset-filled-surface-100-900 p-6">
			...
			<div class="text-sm text-surface-600 dark:text-surface-400">
				<p>This settlement has {settlement.plot.length} plot{settlement.plot.length === 1 ? '' : 's'}.</p>
			</div>
		</div>
	{/if}
