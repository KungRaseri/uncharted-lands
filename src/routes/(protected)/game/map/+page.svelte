<script lang="ts">
	...
	async function fetchRegions(centerX: number, centerY: number, radius: number = 1) {
		...
		try {
			const response = await fetch(`/api/regions/${data.world.id}?centerX=${centerX}&centerY=${centerY}&radius=${radius}`);
		...
		<div class="flex justify-center">
			<div class="card preset-filled-primary-500 p-3 text-sm">
				<p class="font-semibold">
					Viewing center: ({centerCoords.x}, {centerCoords.y}) | 
					Loaded regions: {regions.length} | 
