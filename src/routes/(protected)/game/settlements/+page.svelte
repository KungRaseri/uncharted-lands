...
<div class="max-w-7xl mx-auto p-6 space-y-6">
		...
		<div class="flex items-center gap-4">
			...
			<div class="flex items-center gap-2">
				<button
					...
					class="btn btn-sm {viewMode === 'grid' ? 'preset-filled-primary-500' : 'preset-tonal-surface-500'} rounded-md"
				>
					Grid
				</button>
				<button
					onclick={() => viewMode = 'list'}
					class="btn btn-sm {viewMode === 'list' ? 'preset-filled-primary-500' : 'preset-tonal-surface-500'} rounded-md"
				>
					List
				</button>
			</div>
	...
	{#if filteredSettlements.length === 0}
		<div class="card preset-filled-surface-100-900 p-12 text-center">
			...
			<p class="text-surface-600 dark:text-surface-400">
				{searchTerm ? 'Try a different search term' : 'Settlements will appear here once you establish them in the world'}
			</p>
		</div>
			...
			{#each filteredSettlements as settlement}
				...
				>
					<div class="flex items-start justify-between mb-4">
						<div class="flex items-center gap-2">
							...
							<div>
								...
								<p class="text-xs text-surface-500 dark:text-surface-500 font-mono group-hover:text-white/60">
					...
					<div class="space-y-3">
						...
						<!-- Resources -->
						<div class="pt-3 border-t border-surface-300 dark:border-surface-700 group-hover:border-white/20">
							<div class="flex items-center gap-2 mb-2 text-sm text-surface-600 dark:text-surface-400 group-hover:text-white/80">
								<Package size={14} />
								<span>Resources</span>
							</div>
					...
					<tbody>
						{#each filteredSettlements as settlement}
							<tr class="hover:preset-tonal-surface-500">
								<td>
									<a href="/game/settlements/{settlement.id}" class="font-semibold hover:text-primary-500">
										{settlement.name}
									</a>
