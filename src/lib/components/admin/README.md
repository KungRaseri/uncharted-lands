...
```svelte
<StatsCard label="Total Servers" value={42} icon={Server} href="/admin/servers" />
```
...
```svelte
<PageHeader title="Servers" description="Manage game servers" icon={Server}>
	{#snippet actions()}
...
```svelte
<DetailHeader title={server.name} subtitle={server.id} icon={Server}>
	{#snippet metadata()}
...
<div class="space-y-4">
	...
	<div class="card preset-filled-surface-100-900">
		{#if filteredItems.length === 0}
			<EmptyState icon={ItemIcon} title="No items found" message="Try a different search" />
		{:else}
			<!-- Table or grid -->
		{/if}
	</div>
...
<div class="space-y-6">
	...
	<DetailHeader title={data.item.name} subtitle={data.item.id} icon={ItemIcon}>
		{#snippet metadata()}
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
	<StatsCard label="Total Items" value={stats.items} icon={Icon} href="/admin/items" />
	<!-- More cards... -->
</div>
```
